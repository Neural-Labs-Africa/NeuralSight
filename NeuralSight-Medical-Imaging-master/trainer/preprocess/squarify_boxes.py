...
from merge_boxes_v1 import (create_split_for_centernet,
                            create_test_file_centernet)
from utils import (INPUT_PATH, OUTPUT_PATH, get_train_test_image_sizes,
                   read_single_image, show_resized_image)
...
def squarify_boxes(split_file, div_val):
    ...
    print(
        "AR Min: {:.2f} AR Max: {:.2f} AR Mean: {:.2f}".format(
...
if __name__ == "__main__":
    ...
    create_split_for_centernet(
        initial_file[:-4] + "_sqr.csv", training_img_directory, out_folder
    )
