...
from merge_boxes_v1 import create_split_for_centernet, create_test_file_centernet
...
def extract_boxes_for_rads(init_train_file):
    ...
    train = train[
        (~train["image_id"].isin(exclude_ids)) | (train["image_id"].isin(empty_images))
    ]
    ...
    print(
        "IDs to use: {} Empty IDs: {}".format(
            len(train["image_id"].unique()), len(empty_images)
        )
    )
...
if __name__ == "__main__":
    ...
    create_split_for_centernet(
        init_train_file[:-4] + "_removed_rads.csv", training_img_directory, out_folder
    )
