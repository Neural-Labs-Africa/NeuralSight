...
from utils import (INPUT_PATH, OUTPUT_PATH, get_classes_array, get_color,
                   read_single_image, show_resized_image_16bit)


def draw_boxes(image_id, boxes, scores, labels):
    image = read_single_image(
        INPUT_PATH + "train_png_16bit_div_4/{}.png".format(image_id)
    )
    ...
    for i, b in enumerate(boxes):
        ...
        image = cv2.rectangle(
            image, (b[0], b[1]), (b[2], b[3]), color=color, thickness=thickness
        )
...
def merge_boxes_center_net(div_scale=4, iou_same=0.4):
    ...
    out = open(
        OUTPUT_PATH + "boxes_description_iou_{}_div_{}.csv".format(iou_same, div_scale),
        "w",
    )
